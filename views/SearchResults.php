                            ...
                            <div class = "row py-5 align-center">
                                ...
                                if (isset($searchResults)) {
                                    foreach ($searchResults as $user) {
                                        ...
                                        echo '<form action="/servercwk2016208/index.php/LoginController/followUser" method="POST">';
                                        echo '<img name="image" src="' . $user->getprofilePic() . '" class=" custom-image img-fluid img-thumbnail card-img-top img-rounded" alt="profile picture" />';
                                        echo'<figcaption class="p-4 card-img-bottom">';
                                        echo '<h2 class="h5 font-weight-bold mb-2 font-italic"><a href="/servercwk2016208/index.php/LoginController/loadPublicProfile?username=' . $user->getuserName() . '"><lable class="name">' . $user->getfirstName() . '  ' . $user->getlastName() . '</lable></a></h2>';
                                        echo'<lable class="mb-0 text-small text-muted font-italic"><i class = "fa fa-music mr-2"></i>' . $user->getgenres() . '</lable>';
                                        echo ' </figcaption>';

                                        //displays only if the user is not followed
                                        if (isset($followedUsers)) {
                                            if (in_array($user->getuserName(), $followedUsers) != true) {
                                                echo '<button type ="submit"  class="btn btn-success btn-block">Follow</button>';
                                            } else {
                                                echo '<button type ="submit"  class="btn btn-danger btn-block" id="unfollow" name="unfollow" value="unfollow">Unfollow</button>';
                                            }
                                        }
                                        //hides after clicking the button
                                        if (isset($userAdded)) {
                                            if (in_array($user->getuserName(), $userAdded) != true) {
                                                echo '<button type ="submit" class="btn btn-success btn-block float-bottom "  >Follow</button>';
                                            } else {
                                                echo '<button type ="submit"  class="btn btn-danger btn-block" id="unfollow" name="unfollow" value="unfollow">Unfollow</button>';
                                            }
                                        }
                                        echo '<input type="hidden" id="username" name="username" value =' . $user->getuserName() . '>';
                                        echo '</form>';
                                        echo'</figure>';
                                        echo'</div> ';
                                        echo'<div class="separator px-3">  </div>';
                                    }
                                }
                                ?>
                            </div>
